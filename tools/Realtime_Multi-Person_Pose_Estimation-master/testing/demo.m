...
cat_dict = {{'jumping', 495},{'phoning',457},{'playinginstrument',619},{'reading',530},...
    {'ridingbike',578},{'ridinghorse',534},{'running',561},{'takingphoto',456},...
    {'usingcomputer',476},{'walking',597}};
%%
close all;
savePath = 'result_voc_trainval';
for i=1:length(cat_dict)
    cat_name = cat_dict{i}{1};
    cat_num = cat_dict{i}{2};
    pics = dir(['/space3/water/datasets/VOCtrainval/Bbox_test/' cat_name]);
    pics = pics(3:end);
    
    for m = 1:cat_num
    im_name = pics(m).name;
    %im_name = ['/space3/water/datasets/VOCtrainval/Bbox_test/' im_name];
